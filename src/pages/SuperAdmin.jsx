...
// Creation et gestion complete des entreprises
import { useEffect, useState } from 'react'
import { supabase } from '../lib/supabase'
import { useAuth } from '../hooks/useAuth'
import { PLANS, SECTEURS } from '../lib/modules'
import { MODULES_REGISTRY } from '../modules/registry'

const DEPARTEMENTS_STARTER = ['menage','maintenance','accueil','admin','urgence']
const DEPARTEMENTS_HOTEL = ['petit_dejeuner','restauration','bar','cuisine','room_service','conciergerie','bagagerie','voiturier','securite','spa_bien_etre','piscine','blanchisserie','etages','gouvernante','direction','commercial','comptabilite','rh','evenementiel','technique','jardin_exterieur','nuit']
const TOUS_DEPARTEMENTS = [...DEPARTEMENTS_STARTER, ...DEPARTEMENTS_HOTEL]
...
export default function SuperAdmin() {
  ...
  function ouvrirCreation() {
    const modsDefaut = PLAN_MODULES.starter
    setForm({
      nom: '', slug: '', secteur: 'hotel', plan: 'starter',
      prix_mensuel: 29, max_utilisateurs: 10, actif: true,
      modules_selectionnes: [...modsDefaut],
      departements_selectionnes: [...DEPARTEMENTS_STARTER],
  ...
  function ouvrirEdition(ent) {
    ...
    fetchEntModules(ent.id).then(() => {
      ...
      setForm({
        ...
        modules_selectionnes: mods, departements_selectionnes: [],
  ...
  function changerPlan(plan) {
    ...
    const depts = plan === 'starter' ? [...DEPARTEMENTS_STARTER] : [...DEPARTEMENTS_STARTER, ...DEPARTEMENTS_HOTEL.slice(0, 10)]
    setForm(f => ({
      ...f, plan,
  ...
  function toggleDept(deptId) {
    setForm(f => {
      ...
      return { ...f, departements_selectionnes: sel.includes(deptId) ? sel.filter(d => d !== deptId) : [...sel, deptId] }
    })
  }
  ...
  async function sauvegarder() {
    ...
    try {
      const entData = {
        nom: form.nom, slug: form.slug || form.nom.toLowerCase().replace(/\s+/g, '-'),
      ...
      if (form.modules_selectionnes?.length > 0) {
        const inserts = form.modules_selectionnes.map(modId => ({
          entreprise_id: entId, module_id: modId, actif: true, activated_at: new Date().toISOString()
        }))
        await supabase.from('entreprise_modules').upsert(inserts, { onConflict: 'entreprise_id,module_id' })
      }
      if (!editEntreprise && form.departements_selectionnes?.length > 0) {
        const depts = form.departements_selectionnes.map(code => ({
          entreprise_id: entId,
          nom: code.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
          code, actif: true
        }))
        await supabase.from('departements').insert(depts)
      }
      ...
      setMsg({ type: 'success', text: editEntreprise ? 'Entreprise modifiee !' : 'Entreprise creee !' })
      setShowForm(false)
      fetchData()
    } catch (e) {
    ...
    return (
      <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <button onClick={() => setShowForm(false)} style={{ border: '1px solid #E5E7EB', background: '#fff', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13 }}>
            Retour
          </button>
        ...
        <div style={{ display: 'grid', gap: 20 }}>
          <Section titre="Informations entreprise">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Secteur">
                <select style={inputStyle} value={form.secteur} onChange={e => setForm(f => ({ ...f, secteur: e.target.value }))}>
              ...
              <Field label="Email contact"><input style={inputStyle} value={form.email_contact} onChange={e => setForm(f => ({ ...f, email_contact: e.target.value }))} placeholder="contact@hotel.fr" /></Field>
              ...
              {allMods.map(m => {
                ...
                return (
                  <button key={m.id} onClick={() => toggleModule(m.id)} style={{
                    ...
                    background: sel ? (m.couleur || '#3B82F6') + '18' : '#fff',
                    color: sel ? (m.couleur || '#3B82F6') : '#6B7280',
          ...
          {!editEntreprise && (
            <Section titre="Departements de base">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {TOUS_DEPARTEMENTS.map(d => {
                  const sel = (form.departements_selectionnes || []).includes(d)
                  const isStarter = DEPARTEMENTS_STARTER.includes(d)
                  return (
                    <button key={d} onClick={() => !isStarter && toggleDept(d)} style={{
                      padding: '5px 12px', borderRadius: 16,
                      border: '1.5px solid ' + (sel ? '#3B82F6' : '#E5E7EB'),
                      background: sel ? '#EFF6FF' : '#fff', color: sel ? '#3B82F6' : '#6B7280',
                      cursor: isStarter ? 'default' : 'pointer', fontSize: 12,
                    }}>
                      {d.replace(/_/g, ' ')} {isStarter ? '(socle)' : ''}
                    </button>
                  )
                })}
              </div>
          ...
          {!editEntreprise && (
            <Section titre="Premier administrateur (optionnel)">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <Field label="Email admin"><input style={inputStyle} value={form.admin_email} onChange={e => setForm(f => ({ ...f, admin_email: e.target.value }))} placeholder="jean@hotel.fr" /></Field>
              ...
              {form.admin_email && <div style={{ marginTop: 8, fontSize: 12, color: '#6B7280', background: '#F9FAFB', padding: '8px 12px', borderRadius: 8 }}>Mot de passe temporaire genere : Velor2024!</div>}
            </Section>
          )}
        </div>
      ...
      {onglet === 'entreprises' && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div key={e.id} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#6B7280', marginTop: 3 }}>{e.secteur} - {e.max_utilisateurs || '?'} users max</div>
                  </div>
            ...
            {entreprises.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: '#9CA3AF' }}>Aucune entreprise.</div>}
          </div>
        </div>
      )}
                ...
                {modules.map(m => (
                  <tr key={m.id} style={{ borderBottom: '1px solid #E5E7EB' }}>
                    ...
                    <td style={{ padding: '8px 14px' }}>
                      <span style={{ background: (PLAN_COLORS[m.plan_minimum] || '#6B7280') + '22', color: PLAN_COLORS[m.plan_minimum] || '#6B7280', borderRadius: 10, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>{m.plan_minimum}</span>
                    </td>
                    <td style={{ padding: '8px 14px' }}>
                      <span style={{ color: m.actif ? '#10B981' : '#EF4444', fontWeight: 700 }}>{m.actif ? 'Oui' : 'Non'}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {Object.values(PLANS).map(p => (
              <div key={p.id} style={{ background: '#fff', border: '2px solid ' + p.couleur + '44', borderRadius: 12, padding: 20 }}>
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: 11, color: '#6B7280', fontWeight: 700, marginBottom: 4 }}>MODULES INCLUS</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {(PLAN_MODULES[p.id] || []).map(m => (
                      <span key={m} style={{ background: p.couleur + '18', color: p.couleur, borderRadius: 8, padding: '2px 8px', fontSize: 11 }}>{m}</span>
                    ))}
                    {!PLAN_MODULES[p.id] && <span style={{ color: '#9CA3AF', fontSize: 12 }}>Tous les modules</span>}
                  </div>
                </div>
              </div>
            ))}
