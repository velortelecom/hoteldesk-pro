...
Deno.serve(async (req: Request) => {
  ...
  if (!userId || !prenom || !nom) {
    return corsResponse({ success: false, error: 'missing_required_fields' }, 400);
  }
  ...
  return corsResponse({ success: true, user_id: data.id, prenom: data.prenom, nom: data.nom, entreprise_id: data.entreprise_id }, 200);
});
