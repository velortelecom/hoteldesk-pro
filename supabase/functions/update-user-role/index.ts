...
Deno.serve(async (req: Request) => {
  ...
  if (!userId || !newRole) return corsResponse(req, { success: false, error: 'missing_required_fields' }, 400);
  if (!isAllowedRole(newRole)) return corsResponse(req, { success: false, error: 'invalid_role' }, 400);
  ...
  if (!callerIsSuperAdmin && newRole === 'admin') {
    return corsResponse(req, { success: false, error: 'admin_role_reserved' }, 403);
  }
  ...
  await recordAuditEvent(supabase, {
    ...
    type_cible: 'profile',
    cible_id: userId,
    description: 'Modification rôle utilisateur',
    metadonnees: { new_role: newRole },
    adresse_ip: req.headers.get('x-forwarded-for')?.split(',')[0].trim() ?? null,
    user_agent: req.headers.get('user-agent'),
  });

  return corsResponse(req, { success: true, user_id: userId, role: data.role, entreprise_id: data.entreprise_id }, 200);
});
