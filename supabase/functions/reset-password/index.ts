...
Deno.serve(async (req: Request) => {
  ...
  const temp_password = generateTempPassword(16);
  const { data, error } = await supabase.auth.admin.updateUserById(userId, { password: temp_password });
  if (error || !data.user) return corsResponse(req, { success: false, error: error?.message || 'reset_failed' }, 500);
  ...
  return corsResponse(req, { success: true, user_id: userId, email, temp_password }, 200);
});
